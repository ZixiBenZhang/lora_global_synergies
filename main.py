...
from transformers import (
    AutoConfig,  # for encapsulating configs
    AutoModelForSequenceClassification,  # for loading model from pre-trained
    AutoTokenizer,  # for loading tokenizer
    DataCollatorWithPadding,  # for padding the text if needed
    EvalPrediction,  # for typing: the output dataclass of models
    HfArgumentParser,  # for parsing defined arguments
    PretrainedConfig,  # for checking label's order
    Trainer,  # for training, evaluating, predicting
    TrainingArguments,  # for loading basic arguments of training
    default_data_collator,  # data collator that do nothing
    set_seed,  # for setting the random seed for training
)
...
def load_data(training_args: TrainingArguments, data_args: DataTrainingArguments):
    ...
    if data_args.task_name is not None:
        ...
        datasets_ = load_dataset("glue", data_args.task_name)
    else:
        ...
        if data_args.train_file.endswith(".csv"):
            ...
            datasets_ = load_dataset("csv", data_files=data_files)
        else:
            # Loading a dataset from local json files
            datasets_ = load_dataset("json", data_files=data_files)
...
def main():
    ...
    datasets_ = load_data(training_args, data_args)

    # Get label list and number of labels
    if data_args.task_name is not None:
        is_regression = data_args.task_name == "stsb"
        if not is_regression:
            label_list = datasets_["train"].features["label"].names
            num_labels = len(label_list)
        else:
            num_labels = 1
    else:
        # Trying to have good defaults here, don't hesitate to tweak to your needs.
        is_regression = datasets_["train"].features["label"].dtype in ["float32", "float64"]
        if is_regression:
            num_labels = 1
        else:
            # A useful fast method:
            # https://huggingface.co/docs/datasets/package_reference/main_classes.html#datasets.Dataset.unique
            label_list = datasets_["train"].unique("label")
    ...
    # Set PET params to trainable, freeze other params
    ...
    if data_args.task_name is not None:
        ...
        non_label_column_names = [name for name in datasets_["train"].column_names if name != "label"]
    ...
    if training_args.do_eval:
        ...
        if data_args.task_name == "mnli":
            ...
            eval_datasets.append(datasets_["validation_mismatched"])
    ...
    if training_args.do_predict:
        ...
        if data_args.task_name == "mnli":
            tasks.append("mnli-mm")
            test_datasets.append(datasets_["test_mismatched"])
