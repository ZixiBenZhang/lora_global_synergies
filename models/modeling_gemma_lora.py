...
from .configuration_gemma_lora import GemmaLoraConfig
...
class GemmaLoraMLP(nn.Module):
    def __init__(self, config, layer_id: int = 0):
        ...
        self.gate_proj = LoraLinear(
            self.hidden_size, self.intermediate_size, config=layer_lora_config["w0"], bias=False
        )
        self.up_proj = LoraLinear(
            self.hidden_size, self.intermediate_size, config=layer_lora_config["w1"], bias=False
        )
        self.down_proj = LoraLinear(
            self.intermediate_size, self.hidden_size, config=layer_lora_config["w2"], bias=False
        )
...
# Q, K, V, O in here
class GemmaLoraAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    # Ignore copy
    def __init__(self, config: GemmaLoraConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
GEMMA_ATTENTION_CLASSES = {
    "eager": GemmaLoraAttention,
    # "flash_attention_2": GemmaLoraFlashAttention2,
    # "sdpa": GemmaLoraSdpaAttention,
}


# Copied from transformers.models.llama.modeling_llama.LlamaDecoderLayer with LLAMA->GEMMA,Llama->Gemma
class GemmaLoraDecoderLayer(nn.Module):
    def __init__(self, config: GemmaLoraConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = GEMMA_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)

        self.mlp = GemmaLoraMLP(config, layer_id=layer_idx)
...
class GemmaLoraPreTrainedModel(PreTrainedModel):
    config_class = GemmaLoraConfig
...
class GemmaLoraModel(GemmaLoraPreTrainedModel):
    """
    ...
    def __init__(self, config: GemmaLoraConfig):
        ...
        self.layers = nn.ModuleList(
            [GemmaLoraDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class GemmaLoraForCausalLM(GemmaLoraPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = GemmaLoraModel(config)
        self.vocab_size = config.vocab_size
...
class GemmaLoraForSequenceClassification(GemmaLoraPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
