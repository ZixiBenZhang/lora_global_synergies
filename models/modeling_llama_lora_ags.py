...
_CONFIG_FOR_DOC = "LlamaLoraAgsConfig"


ALL_LAYERNORM_LAYERS.append(LlamaRMSNorm)


class LlamaLoraAgsMLP(nn.Module):
...
class LlamaLoraAgsAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: LlamaLoraAgsConfig, layer_idx: int = 0):
        super().__init__()
        self.config = config
...
class LlamaLoraAgsSdpaAttention(LlamaLoraAgsAttention):
    """
...
LLAMA_ATTENTION_CLASSES = {
    "eager": LlamaLoraAgsAttention,
    # "flash_attention_2": LlamaFlashAttention2,
    "sdpa": LlamaLoraAgsSdpaAttention,
}


class LlamaLoraAgsDecoderLayer(nn.Module):
    def __init__(self, config: LlamaLoraAgsConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.layer_idx = layer_idx
        self.self_attn = LLAMA_ATTENTION_CLASSES[config._attn_implementation](
            config=config, layer_idx=self.layer_idx
        )

        self.mlp = LlamaLoraAgsMLP(config, layer_idx)
...
class LlamaLoraAgsPreTrainedModel(PreTrainedModel):
    config_class = LlamaLoraAgsConfig
...
class LlamaLoraAgsModel(LlamaLoraAgsPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`LlamaLoraAgsDecoderLayer`]

    Args:
        config: LlamaLoraAgsConfig
    """

    def __init__(self, config: LlamaLoraAgsConfig):
        ...
        self.layers = nn.ModuleList(
            [
                LlamaLoraAgsDecoderLayer(config, layer_idx)
    ...
    def forward(
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs, residual_ffn = self._gradient_checkpointing_func(
...
class LlamaLoraAgsForCausalLM(LlamaLoraAgsPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config: LlamaLoraAgsConfig):
        super().__init__(config)
        self.model = LlamaLoraAgsModel(config)
        self.vocab_size = config.vocab_size
...
class LlamaLoraAgsForSequenceClassification(LlamaLoraAgsPreTrainedModel):
    def __init__(self, config: LlamaLoraAgsConfig):
        super().__init__(config)
...
class LlamaLoraAgsForQuestionAnswering(LlamaLoraAgsPreTrainedModel):
    base_model_prefix = "transformer"

    # Copied from transformers.models.bloom.modeling_bloom.BloomForQuestionAnswering.__init__ with Bloom->Llama
    def __init__(self, config: LlamaLoraAgsConfig):
        super().__init__(config)
        self.transformer = LlamaLoraAgsModel(config)
...
class LlamaLoraAgsForTokenClassification(LlamaLoraAgsPreTrainedModel):
    def __init__(self, config: LlamaLoraAgsConfig):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = LlamaLoraAgsModel(config)
